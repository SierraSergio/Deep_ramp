#función añadida por mi en la linea 334 para unificar los colores en la detección de objetos y que todos los objetos de la misma clase tengan el mismo color.
...
class Visualizer:
    ...
    def __init__(self, img_rgb, metadata=None, scale=1.0, instance_mode=ColorMode.IMAGE):
        ...
        #función añadida por mi en la linea 334 para unificar los colores en la detección de objetos y que todos los objetos de la misma clase tengan el mismo color.
    def unifica_color_clase(self, pred_classes=None):
        _COLORS = np.array(
            [
                0.000, 0.447, 0.741,
                0.850, 0.000, 0.000,
    ...
    def draw_instance_predictions(self, predictions):
        ...
        if self._instance_mode == ColorMode.IMAGE_BW:
            self.output.img,nuevo_negro = self._create_black_image(
        ...
        colors=self.unifica_color_clase(classes)
        ...
        return self.output,self.output.img,nuevo_negro
    ...
    #añadida por mi esta funcion    
    def _create_black_image(self, mask=None):
        """
        Create a grayscale version of the original image.
        The colors in masked area, if given, will be kept.
        """
        img_black = self.img.astype("f4").min(axis=2)
        img_black = np.stack([img_black] * 3, axis=2)
        s=self.img.shape
        r,g,b=cv2.split(self.img)
        s=(s[0],s[1])
        nuevo=np.zeros((b.shape[0],b.shape[1]),dtype=b.dtype)
        nuevo = np.stack([nuevo] *3, axis=2)
        nuevo[mask]=self.img[mask]
        if mask is not None:
            img_black[mask] = self.img[mask]
        
        return img_black,nuevo
